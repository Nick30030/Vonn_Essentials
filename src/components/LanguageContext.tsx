...
const upsertStoreData = async (payload: Record<string, any>) => {
  try {
  ...
  try {
    if (supabase && typeof supabase.from === "function") {
      const { error } = await supabase.from("store").upsert({ id: "global", ...payload });
      if (error) {
        console.warn("Supabase store sync warning:", error);
      }
    }
  } catch (e) {
...
const syncStoreToSupabase = upsertStoreData;

const loadStoreData = async () => {
  try {
    if (supabase && typeof supabase.from === "function") {
      const { data, error } = await supabase.from("store").select("*").eq("id", "global").maybeSingle();
      if (!error && data && (Array.isArray(data.productsEn) || Array.isArray(data.productsFr))) {
        return data;
      }
      if (error) {
        console.warn("Direct Supabase store load warning:", error);
      }
    }
  } catch (e) {
  ...
  // Fallback to local server API if Supabase query is unavailable
  try {
...
export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Subscribe to realtime store updates from Supabase
          ...
          .on(
            ...
            (payload: any) => {
              if (payload.new) {
                ...
                // Real-time announcement banner sync for ALL users/guests (no auth required)
                if (data.announcement) {
                  localStorage.setItem("vonn_announcement", JSON.stringify(data.announcement));
                  window.dispatchEvent(new Event("vonn_announcement_changed"));
                }
                if (data.productsEn || data.productsFr) {
                  if (Array.isArray(data.productsEn) && data.productsEn.length > 0) {
                    productsEnRef.current = data.productsEn;
                    setProductsEn(data.productsEn);
                    localStorage.setItem("vonn_products_en", JSON.stringify(data.productsEn));
                  ...
                  if (Array.isArray(data.blogsEn)) {
                    setBlogsEn(data.blogsEn);
                    localStorage.setItem("vonn_blogs_en", JSON.stringify(data.blogsEn));
                  }
                  if (Array.isArray(data.blogsFr)) {
                    setBlogsFr(data.blogsFr);
                    localStorage.setItem("vonn_blogs_fr", JSON.stringify(data.blogsFr));
                  }
                  if (data.heroContent) {
  ...
  const updateShippingContent = (en: string[], fr: string[]) => {
    ...
    void upsertStoreData({ shippingEn: en, shippingFr: fr });
  };
  ...
  const updateAboutContent = (en: string[], fr: string[]) => {
    ...
    void upsertStoreData({ aboutEn: en, aboutFr: fr });
  };
  ...
  const updateHeroContent = (data: Partial<HeroContent>) => {
    ...
    void upsertStoreData({ heroContent: updated });
  };
  ...
  const updateShippingSection = (data: Partial<SectionContentData>) => {
    ...
    void upsertStoreData({ shippingSection: updated, shippingEn: updated.contentEn || shippingContentEn, shippingFr: updated.contentFr || shippingContentFr });
  };
  ...
  const updateAboutSection = (data: Partial<SectionContentData>) => {
    ...
    void upsertStoreData({ aboutSection: updated, aboutEn: updated.contentEn || aboutContentEn, aboutFr: updated.contentFr || aboutContentFr });
  };
  ...
  const addBlogPost = (postEn: any, postFr: any) => {
    ...
    void upsertStoreData({ blogsEn: newEn, blogsFr: newFr });
  };
  ...
  const updateAdminPasscode = (newPass: string) => {
    ...
    void upsertStoreData({ adminPasscode: newPass });
  };
  ...
  const updateAdminEmail = (newEmail: string) => {
    ...
    void upsertStoreData({ adminEmail: trimmed });
  };
  ...
  const addProduct = (productEn: Product, productFr: Product) => {
    ...
    // Persist store state directly to Supabase `store` row
    void upsertStoreData({ productsEn: updatedEn, productsFr: updatedFr });
  };
