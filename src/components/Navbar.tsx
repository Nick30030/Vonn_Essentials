...
export default function Navbar({ itemCount, onOpenCart, onOpenTerms }: NavbarProps) {
  ...
  useEffect(() => {
    // Fetch the latest announcement from Supabase DB on mount so every visitor
    // (guest, incognito, fresh browser) sees the admin-published banner even
    // if nothing is cached in localStorage.
    const fetchAnnouncementFromDB = async () => {
      try {
        ...
        // Suppressed — fall back to localStorage-cached value already in state
      }
    };
    void fetchAnnouncementFromDB();
