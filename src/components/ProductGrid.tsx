...
export default function ProductGrid({ onAddToCart }: ProductGridProps) {
  ...
  const filteredProducts = (selectedCollection === "All" || selectedCollection === "Tous")
    ? currentProductsList
    : currentProductsList.filter((p) => {
        if (!p.collection) return false;
        const normCol = p.collection.trim().toLowerCase();
        const normSel = selectedCollection.trim().toLowerCase();
        return normCol === normSel;
      });
  ...
  const customCollections = Array.from(
    new Set(
      currentProductsList
        ...
        .filter((c): c is string => Boolean(c && !baseCollections.some(b => b.toLowerCase() === c.toLowerCase())))
    )
  );
