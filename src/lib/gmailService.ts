...
// Listen for Supabase auth state changes (fallback for sessions arriving via redirect)
supabase.auth.onAuthStateChange((event: string, session: any) => {
  if (!session || !session.user) {
    // Don't wipe GIS-based sessions on Supabase sign-out
    if (cachedGoogleUser === null) notifyListeners();
    return;
  }

  // Only update from Supabase session if we don't already have a GIS session
  if (!cachedAccessToken) {
    const providerToken = (session as any)?.provider_token || (session as any)?.providerAccessToken || null;
    cachedAccessToken = providerToken || null;
    cachedGoogleUser = {
      email: session.user.email || null,
      displayName: session.user.user_metadata?.full_name || session.user.user_metadata?.name || session.user.email || null,
...
// ---------------------------------------------------------------------------
// Google Identity Services (GIS) – Real OAuth2 popup
// ---------------------------------------------------------------------------

/** 
 * Fetches the authenticated Google user's profile from the userinfo endpoint
 * using the access token returned by GIS. This validates the token is real.
 */
async function fetchGoogleUserInfo(accessToken: string): Promise<{
  ...
  if (!res.ok) throw new Error("Failed to verify Google account. Please try again.");
  return res.json();
}

/**
 * Waits for the Google Identity Services script to be ready.
 */
...
/**
 * Opens a REAL Google account picker popup using Google Identity Services.
 * The returned access token is verified against Google's userinfo endpoint –
 * there is no way to spoof the identity without actually signing in.
 */
export const connectGmailAccount = async (targetEmailHint?: string): Promise<ConnectGmailResult> => {
  ...
  if (!clientId) {
    throw new Error(
      "Google Client ID is not configured. Add VITE_GOOGLE_CLIENT_ID to your .env.local and Vercel environment variables."
    );
  }

  return new Promise<ConnectGmailResult>((resolve, reject) => {
    const google = (window as any).google;

    // Build the OAuth2 token client – this opens the real Google account picker popup
    const tokenClient = google.accounts.oauth2.initTokenClient({
      client_id: clientId,
      scope: GMAIL_SCOPES.join(" ") + " openid email profile",
      hint: targetEmailHint || "",
      // Force account selection even if already signed in – prevents skipping
      ...
      callback: async (tokenResponse: any) => {
        try {
          ...
          // Verify the token by fetching the profile from Google – cannot be faked
          ...
          notifyListeners();
          resolve({ user: cachedGoogleUser, accessToken, cancelled: false });
        } catch (err) {
          reject(err);
        }
      },
      error_callback: (err: any) => {
        if (err?.type === "popup_closed") {
          ...
          reject(new Error(err?.message || "Google sign-in failed. Please try again."));
        }
      },
    });

    // Trigger the real Google account picker popup
    tokenClient.requestAccessToken();
  });
};

/**
 * Disconnects the Gmail / GIS session and revokes the access token.
 */
...
export const buildThankYouEmailHtml = (order: OrderDataForPdf, logoBase64: string): string => {
  ...
  const formatCurrency = (val: any): string => {
    ...
    if (typeof val === "number") return isNaN(val) ? "C$0.00" : `C$${val.toFixed(2)}`;
  ...
  const cleanTotal = formatCurrency(order.total);
  const cleanSubtotal = formatCurrency(order.subtotal);
  const cleanShipping = formatCurrency(order.shipping);
  const cleanHst = formatCurrency(order.hst);
...
export const buildRefundProcessingEmailHtml = (data: RefundEmailData, logoBase64: string): string => {
  ...
  const cleanAmount = typeof data.amount === "number" ? data.amount : parseFloat(String(data.amount).replace(/[^\d.-]/g, "")) || 0;
  const formattedAmount = `C$${cleanAmount.toFixed(2)}`;
